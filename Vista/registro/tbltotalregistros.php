...
if($_SESSION['usuarios']['rol'] == 1){
    ...
    $sql="SELECT
        ...
        COUNT(*) as registros,
        CASE WHEN tipo_documento = 'factura' THEN 'FACTURA' ELSE 0 END facturas,
        (select count(*) from registros re where estado = 'enviado' and r.tipo_documento = re.tipo_documento and re.cod_almacen = '$bodega') as enviados,
        (select count(*) from registros rp where estado = 'programado' and r.tipo_documento = rp.tipo_documento and rp.cod_almacen = '$bodega') as programados,
        (select count(*) from registros ren where estado = 'entregado' and r.tipo_documento = ren.tipo_documento and ren.cod_almacen = '$bodega') as entregados,
        (select count(*) from registros rer where estado = 'registrado' and r.tipo_documento = rer.tipo_documento and rer.cod_almacen = '$bodega') as registrados
    FROM registros r
    WHERE r.cod_almacen = '$bodega'
    group by tipo_documento";
}
