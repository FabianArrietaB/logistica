...
session_start();
    ...
    $sql="SELECT
            ...
            FROM
                registros
                WHERE 
                      estado ='programado'  AND cod_almacen = '$bodega'";
    $respuesta = mysqli_query($conexion, $sql);
?>
<table class="table table-sm  dt-responsive nowrap" id="tablaProgramarRegistroDataTable" style="width:100%">
<thead>
  <th>#</th>
  <th>Cod_almacen</th>
  <th>Fecha</th>
      ...
      <td><?php echo $contador++;?></td>
        ...
        <td><?php echo $mostrar['estado'];?>
            <button class="btn btn-warning btn-sm"
                    data-toggle="modal" data-target="#modalAgregarEstadoEnviado"
                    onclick="obtenerEstadoEnviado(<?php echo $mostrar['idRegistro']?>)">
...
<script>
    $(document).ready(function(){
        $('#tablaProgramarRegistroDataTable').DataTable();
    });
</script>